...
for ep_g in range(args.epochs_global):
    ...
    task_id = int((ep_g / args.epochs_global) * args.tasks_global)
    ...
    out_file.write(log_str + "\n")
    out_file.flush()
    print("Classes learned = {}".format(classes_learned))
    print(
        "classification accuracy of global model at round %d: %.3f" % (ep_g, acc_global)
    )
