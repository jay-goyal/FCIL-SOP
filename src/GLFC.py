...
class GLFC_model:
    ...
    def _get_train_and_test_dataloader(self, train_classes, mix):
        if mix:
            self.train_dataset.getTrainData(
                train_classes, self.exemplar_set, self.learned_classes
            )
        else:
    ...
    def _compute_loss(self, indexs, imgs, label):
        ...
        if self.old_model == None:
            ...
            loss_cur = torch.mean(
                w * F.binary_cross_entropy_with_logits(output, target, reduction="none")
            )
    ...
    def Image_transform(self, images, transform):
        ...
        for index in range(1, len(images)):
            data = torch.cat(
                (data, self.transform(Image.fromarray(images[index])).unsqueeze(0)),
                dim=0,
            )
        return data
    ...
    def compute_class_mean(self, images, transform):
        ...
        feature_extractor_output = (
            F.normalize(self.model.feature_extractor(x).detach()).cpu().numpy()
        )
    ...
    def compute_exemplar_class_mean(self):
        ...
        for index in range(len(self.exemplar_set)):
            ...
            class_mean = (
                ...
                + class_mean_ / np.linalg.norm(class_mean_)
            ) / 2
